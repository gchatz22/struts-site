...
CSP comes in many flavors, but the policy we chose to implement is a nonce-based strict-dynamic CSP, which is the most robust policy. This policy requires the browser to check that all scripts have the valid nonce value (which is included in the response header generated by the server) before running them. This ensures that a malicious script not approved (by attaching the nonce value) by the server will not run, thus protecting against XSS attacks. With each response, a new nonce value is generated, so adopting CSP requires implementing a mechanism that automatically attaches the valid nonce value to all `script, link` blocks. Strict CSP also blocks any inline event handlers (`onclick` etc.) and `javascript:` URIs, so adopting this policy requires refactoring by the developers if these were in use. The default recommended policy used is as follows:
...
The specified policy includes a `report-uri` directive. This should not be confused with the report-only and enforcing modes of CSP. In both modes, if the `report-uri` is specified, the browser will send a JSON blob containing details about the violation as a POST request to the specified `report-uri`.
